...
def worker_loop(api_url: str, cores: int, has_gpu: bool, batch_size: int):
    ...
    with concurrent.futures.ProcessPoolExecutor(
        max_workers=cores, initializer=_init_worker_process
    ) as executor:
        while True:
            try:
                resp = httpx.post(
                    f"{api_url}/worker/poll",
                    json={"batch_size": batch_size, "has_gpu": has_gpu},
                    timeout=10.0
                )
                resp.raise_for_status()
                data = resp.json()
                ...
                if not tasks:
                    ...
                    if max_idle_polls is not None and idle_polls >= max_idle_polls:
                        logger.info(
                            "No tasks after %d idle polls; exiting worker.", idle_polls
                        )
                        break
                    time.sleep(2)
                    continue
